...
(function() {
  ...
  window.onerror = function (message, source, lineno, colno, error) {
    ...
    if (/ERR_BLOCKED_BY_CLIENT|MIME|CORS/.test(error?.message || message)) {
      console.warn('Adblocker or extension blocking CDN—using local assets.');
      localStorage.setItem('errorLog', JSON.stringify({ message, source, lineno, colno, timestamp: new Date().toISOString() }));
    }
    return true; // Suppress default browser popup
  };

  // Load Script Utility (with CSP-safe handling)
  ...
  async function initBasicChart() {
    ...
    try { ChartRef.register(ZoomPlugin); } catch {}
    ...
    new ChartRef(ctx, {
      ...
      data: {
        ...
        datasets: [{ label: 'FFlag Changes', data: [10, 20, 15, 25, 30], borderColor: 'rgb(75, 192, 192)', tension: 0.1 }]
      },
      options: {
        responsive: true,
        plugins: {
      ...
      try {
        ...
        if (!ChartRef || !zoomPlugin) {
          try {
            ...
            console.warn('CDN failed, using local.');
            await loadScript('assets/chart.js');
            await loadScript('assets/chartjs-plugin-zoom.js');
          }
          ChartRef = window.Chart;
        ...
        new ChartRef(ctx, {
          ...
          options: { responsive: true, plugins: { legend: { position: 'top' }, zoom: { zoom: { wheel: { enabled: true }, pinch: { enabled: true }, mode: 'x' }, pan: { enabled: true, mode: 'x' } } } }
        });
        loadingP.remove();
        ...
        if (retries < maxRetries) {
          retries++;
          loadingP.textContent = `Retrying... (${retries}/${maxRetries})`;
          setTimeout(loadChartData, retries * 2000);
        } else {
          loadingP.textContent = 'Failed to load chart after retries. Check extensions or network.';
        }
      }
    }
    await loadChartData();
  ...
  function initDebug() {
    document.addEventListener('keydown', e => {
      if (e.ctrlKey && e.shiftKey && e.key === 'D') {
        console.log('Grok Raw Debug:', window.globalData || 'No globalData defined');
